...
class OAuthController extends Controller
{
	...
	$clientId = "(発行されたクライアントIDを設定します)";
	$clientSecret = "(発行されたクライアントシークレットを設定します)";
	$scope = "(scopeをスペース区切りで設定します)";
	...
	$authzReqUrl = "https://oa.connect.auone.jp/net/opi/hny_oauth_rt_net/cca" . "?ID=OpenAPIAcpt&response_type=code&client_id=" . $clientId
. "&redirect_uri=" . $redirectUrl . "&scope=" . urlencode($scope);
	$tokenReqUrl = "https://oa.connect.auone.jp/net/opi/hny_oauth_rt_net/cca" . "?ID=OpenAPITokenCodeReq";
	$tokenParams = "grant_type=authorization_code&redirect_uri=" . $redirectUrl . "&client_id=" . $clientId . "&client_secret=" . $clientSecret;
	// セッション処理開始
	...
	case 'redirect':
		...
		if(!empty($state_query) && $state_query != $state){
			$response_html = '<html><body>Unmatch['. $query('state') . ' <=> '. $state . ']<br /><br /><a href="'. $_SERVER["SCRIPT_NAME"] .'">Retry</a></body></html>"';
			return new Response( $response_html );
		}
		// サーバエラー時はメッセージ表示して終了
		$error_query = $query->get('error'); $error_description_query = $query->get('error_description');
		if(!empty($error_query) || !empty($error_description_query)){
			$error_message = $query->get('error') . '[' . $query->get('error_description') .']';
		...
		// 取得データをセッション/変数格納
		$accessToken = $jobj->access_token; $refreshToken = $jobj->refresh_token; $refreshLimit = $jobj->expires_in;
		break;
	// === リトライ時 ===
	case 'retry':
		$session->clear();
	...
	$pass_array = array('state' => $state, 'code' => $code, 'accessToken' => $accessToken, 'refreshToken' => $refreshToken, 'refreshLimit' => $refreshLimit);
