...
(function() {
	...
	this.JankenTitle.prototype.loading = function(cb_func) {
		if (this.isLoaded) {
			...
			for (var i = 0, ln = images.length; i < ln; i++) {
				loader.addImageRequest("/bundles/kanaheigame/GameJanken/images/" + images[i]);
			}
	...
	this.Character.prototype.show = function(index, cb_func) {
		...
		chara.element.css("background", "url(/bundles/kanaheigame/GameJanken/images/" + charaString + ") no-repeat center top");
	...
	this.JankenMain = function(baseScale) {
		...
		loadXML("/bundles/kanaheigame/GameJanken/config.xml", function(xml) {
