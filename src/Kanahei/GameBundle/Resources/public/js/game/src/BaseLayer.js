var BaseLayer = cc.Layer.extend({
    ...
    ctor:function (parent, playInfo) {
        ...
        //正答を初期化
        for(var i=0; i< this.playInfo.kanahei_LIMIT; i++){
...
//            cc.log("IllustLayer.setIllustFullTargetRect(" + 0 + ", " + Footer._rect.height + ", " + width + ", " + height + ")");
			...
			mainMenu.setPosition(650, this.kanahei_Y - 25);
			this.menu = mainMenu;
	...
	dispSaveData:function(){
		...
		if(this.playInfo._playData._touchData){
			for(var i in this.playInfo._playData._touchData){
				...
				var objs = this.playInfo.kanahei_POINT_DATA;
				var trueFlg = false;
    ...
    initStarsAndHearts:function(){
        self = this;
        this.stars = new Stars(self,this.playInfo.kanahei_LIMIT);
...
//			}
	...
	dispHint:function(){
		...
		// 正解ポイントの取得
		var apObj = this.playInfo.kanahei_POINT_DATA;
		// ヒントになる座標を取得
